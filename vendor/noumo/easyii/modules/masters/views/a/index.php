<?php
use yii\easyii\modules\masters\models\Masters;
use yii\helpers\Html;
use yii\helpers\Url;

$this->title = Yii::t('easyii/masters', 'Мастера');
...
<?php if($data->count > 0) : ?>
    <table class="table table-hover">
        <thead>
            <tr>
                ...
                <th width="120"><?= Yii::t('easyii', 'Рейтинг') ?></th>
                <th width="100"><?= Yii::t('easyii', 'Доступность') ?></th>
                <th width="120"></th>
            </tr>
        </thead>
        <tbody>
    <?php foreach($data->models as $item) : ?>
            <tr data-id="<?= $item->primaryKey ?>">
                ...
                <td><?= $item->rating ?></td>
                <td class="status">
                    <?= Html::checkbox('', $item->status == Masters::STATUS_ON, [
                        'class' => 'switch',
                        'data-id' => $item->status,
